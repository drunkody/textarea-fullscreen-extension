...
export function useKeyboardShortcut(
  key: string | undefined, // ✅ Allow undefined
  callback: () => void,
  ...
  useEffect(() => {
    // ✅ ADDED: Skip if no key provided
    if (!key) {
      logger.warn('[useKeyboardShortcut] No key provided, skipping listener');
      return;
    }
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (keyMatch && ctrlMatch && altMatch && shiftMatch) {
        logger.info(`[useKeyboardShortcut] Triggered: ${key}`, { 
          ctrl: e.ctrlKey, 
          alt: e.altKey, 
          shift: e.shiftKey 
        });
